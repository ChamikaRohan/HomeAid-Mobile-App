const getSlider = () => { 
  const value = {
    "sliders": [
      {
        "id": "clw69mgik04rq07piw53vk3l7",
        "name": "Slider 1",
        "image": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clw8wqupp02o808ph1pae8yi4"
        }
      },
      {
        "id": "clw8vcqpm014a07pkybbis957",
        "name": "Slider 2",
        "image": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clw8ws74502j407pk9k0xx8nm"
        }
      }
    ]
    }
  return value
}

const getCategories = () => { 
  const value = {
    "categories": [
      {
        "id": "clw8x58q0031508phgnvafw68",
        "name": "Cleaning",
        "icon": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clw8x52c502sw07pk8t35ykfx"
        }
      },
      {
        "id": "clw8x63bk032j08phg9w1cji7",
        "name": "Repairing",
        "icon": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clw8x5xgn02u707pkbdn8qfzu"
        }
      },
      {
        "id": "clw8x6w2102wb07pkpqtexgc5",
        "name": "Painting",
        "icon": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clw8x6k1m033d08phjov5pysr"
        }
      },
      {
        "id": "clw8x7k7v02xi07pki5vfe1ai",
        "name": "Shifting",
        "icon": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clw8x7elv02x207pkd5kcoxv1"
        }
      }
    ]
    }
  return value
}

const getBusinessList = () => { 
  const value = {
    "businessLists": [
      {
        "id": "clwatkdek02wx07o6rkamcevt",
        "name": "Kavindu Perera",
        "email": "kavindu99@gmail.com",
        "contactPerson": "Washrooms Cleaning",
        "category": {
          "name": "Cleaning"
        },
        "address": "Hanthana, Kandy",
        "about": "Kavindu Perera offers meticulous washroom and bathroom cleaning services in the serene vicinity of Hanthana, Kandy. With a commitment to excellence and attention to detail, Kavindu ensures that every nook and cranny is spotless, leaving your space fresh and hygienic. Whether it's residential or commercial, Kavindu's services are tailored to meet your needs, providing a sparkling clean environment for your comfort and satisfaction. Trust Kavindu Perera for a pristine washroom experience that exceeds expectations.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwathv7j02v007plsu70d4x0"
        }
      },
      {
        "id": "clwatmtle02z107plgiw827iz",
        "name": "Nimesha De Silva",
        "email": "n.de.silva@gmail.com",
        "contactPerson": "House painting",
        "category": {
          "name": "Painting"
        },
        "address": "Hapugala, Galle",
        "about": "Nimesha De Silva brings her expertise in house painting to the tranquil neighborhood of Hapugala in Galle. With a keen eye for detail and a passion for transforming spaces, Nimesha ensures that every stroke of paint adds vibrancy and life to your home. Whether it's a fresh coat to revitalize your interiors or a bold color choice to make a statement, Nimesha's meticulous workmanship and dedication to customer satisfaction guarantee a stunning finish that enhances the beauty of your home in Hapugala, Galle. Trust Nimesha De Silva for professional house painting services that exceed expectations.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwatm5nf02ym07pl5atbz3be"
        }
      },
      {
        "id": "clwatovd9031c07o6f37xq3wo",
        "name": "Nimal Alponsu",
        "email": "alponsu.nimal@gmail.com",
        "contactPerson": "Delivery Service",
        "category": {
          "name": "Shifting"
        },
        "address": "Kadawatha, Colombo",
        "about": "Mimal Alponsu offers reliable delivery services for any item in the bustling city of Kadawatha, Colombo. With a focus on efficiency and timeliness, Mimal ensures that your packages, parcels, and goods reach their destination safely and on schedule. Whether it's a small package or a bulky item, Mimal's commitment to customer satisfaction guarantees a seamless delivery experience. Trust Mimal Alponsu for swift and dependable delivery services that cater to your needs in Kadawatha, Colombo.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwatnocy02zq07o6ll3ktyr3"
        }
      },
      {
        "id": "clwdoia9f17cu07po52uko6ad",
        "name": "Vimal Fernando",
        "email": "nimalperera@example.com",
        "contactPerson": " Cleaning Services",
        "category": {
          "name": "Cleaning"
        },
        "address": "123, Main Street, Colombo",
        "about": "Vimal Fernando is dedicated to providing top-notch cleaning services with a smile. With years of experience, Nimal ensures your space shines brighter than ever.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdohlsb17by07po14ut68jn"
        }
      },
      {
        "id": "clwdojxkn17vy07pi7mw2ku74",
        "name": "Amal Wijesinghe",
        "email": "malwijesinghe@example.com",
        "contactPerson": "Painting Services",
        "category": {
          "name": "Painting"
        },
        "address": "789, Beach Road, Galle",
        "about": "Amal Wijesinghe is the painting expert you can trust. With a keen eye for detail and a passion for perfection, Amal transforms spaces into works of art.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdojkw317vj07pi69f6qv2x"
        }
      },
      {
        "id": "clwdol5yr17xi07pixdah9khk",
        "name": "Saman Jayasinghe",
        "email": "samanjayasinghe@example.com",
        "contactPerson": "Cleaning Services",
        "category": {
          "name": "Cleaning"
        },
        "address": "101, Lakeview Drive, Nuwara Eliya",
        "about": "Saman Jayasinghe takes pride in providing exceptional cleaning services. With attention to every corner and crevice, Saman ensures your space sparkles.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdokwo017ga07popkpih68l"
        }
      },
      {
        "id": "clwdomq6x17il07porxidy920",
        "name": "Kusal Rajapaksa",
        "email": "kusalrajapaksa@example.com",
        "contactPerson": "Delivery Services",
        "category": {
          "name": "Shifting"
        },
        "address": "222, Hillside Avenue, Matale",
        "about": "Kusal Rajapaksa is your go-to delivery expert. With a commitment to efficiency and reliability, Kusal ensures your parcels are delivered with care.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdom8z217zl07pimvvipat3"
        }
      },
      {
        "id": "clwdop4cy185507pidorqcxos",
        "name": "Tharindu Alwis",
        "email": "tharinduwickrama@example.com",
        "contactPerson": "Painting Services",
        "category": {
          "name": "Painting"
        },
        "address": "333, Riverside Road, Anuradhapura",
        "about": "Tharindu Wickramasinghe is passionate about painting. With creative flair and precision, Tharindu transforms walls into masterpieces.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdooq9g17lz07pom4t7hofo"
        }
      },
      {
        "id": "clwdoq2i2187407pi5euz0zvs",
        "name": "Dilshan Bandara",
        "email": "dilshanbandara@example.com",
        "contactPerson": "Repair Services",
        "category": {
          "name": "Repairing"
        },
        "address": "888, Avenue Street, Negombo",
        "about": "Dilshan Bandara is your reliable repair expert. With years of experience and a knack for fixing things, Dilshan ensures your appliances and devices are back in working order.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdoptnw17ng07pouzhvy7md"
        }
      },
      {
        "id": "clwdou2kz17ra07po095lk8b0",
        "name": "Upul Gunasekara",
        "email": "pulgunasekara@example.com",
        "contactPerson": "Delivery Services",
        "category": {
          "name": "Shifting"
        },
        "address": "555, Mountain View Drive, Badulla",
        "about": "Upul Gunasekara is your trusted delivery partner. With speed and reliability, Upul ensures your packages are delivered with care and efficiency.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdots4017qv07pocwnz3qa0"
        }
      }
    ]
    }
  return value
}

const getBusinessListByCategory = (category) => {

  const Cleaning = {
    "businessLists": [
      {
        "id": "clwatkdek02wx07o6rkamcevt",
        "name": "Kavindu Perera",
        "email": "kavindu99@gmail.com",
        "contactPerson": "Washrooms Cleaning",
        "category": {
          "name": "Cleaning"
        },
        "address": "Hanthana, Kandy",
        "about": "Kavindu Perera offers meticulous washroom and bathroom cleaning services in the serene vicinity of Hanthana, Kandy. With a commitment to excellence and attention to detail, Kavindu ensures that every nook and cranny is spotless, leaving your space fresh and hygienic. Whether it's residential or commercial, Kavindu's services are tailored to meet your needs, providing a sparkling clean environment for your comfort and satisfaction. Trust Kavindu Perera for a pristine washroom experience that exceeds expectations.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwathv7j02v007plsu70d4x0"
        }
      },
      {
        "id": "clwdoia9f17cu07po52uko6ad",
        "name": "Vimal Fernando",
        "email": "nimalperera@example.com",
        "contactPerson": " Cleaning Services",
        "category": {
          "name": "Cleaning"
        },
        "address": "123, Main Street, Colombo",
        "about": "Vimal Fernando is dedicated to providing top-notch cleaning services with a smile. With years of experience, Nimal ensures your space shines brighter than ever.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdohlsb17by07po14ut68jn"
        }
      },
      {
        "id": "clwdol5yr17xi07pixdah9khk",
        "name": "Saman Jayasinghe",
        "email": "samanjayasinghe@example.com",
        "contactPerson": "Cleaning Services",
        "category": {
          "name": "Cleaning"
        },
        "address": "101, Lakeview Drive, Nuwara Eliya",
        "about": "Saman Jayasinghe takes pride in providing exceptional cleaning services. With attention to every corner and crevice, Saman ensures your space sparkles.",
        "images": {
          "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdokwo017ga07popkpih68l"
        }
      }
    ]
    }
  
    const Repairing = {
      "businessLists": [
        {
          "id": "clwdoq2i2187407pi5euz0zvs",
          "name": "Dilshan Bandara",
          "email": "dilshanbandara@example.com",
          "contactPerson": "Repair Services",
          "category": {
            "name": "Repairing"
          },
          "address": "888, Avenue Street, Negombo",
          "about": "Dilshan Bandara is your reliable repair expert. With years of experience and a knack for fixing things, Dilshan ensures your appliances and devices are back in working order.",
          "images": {
            "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdoptnw17ng07pouzhvy7md"
          }
        }
      ]
    }

    const Painting = {
      "businessLists": [
        {
          "id": "clwatmtle02z107plgiw827iz",
          "name": "Nimesha De Silva",
          "email": "n.de.silva@gmail.com",
          "contactPerson": "House painting",
          "category": {
            "name": "Painting"
          },
          "address": "Hapugala, Galle",
          "about": "Nimesha De Silva brings her expertise in house painting to the tranquil neighborhood of Hapugala in Galle. With a keen eye for detail and a passion for transforming spaces, Nimesha ensures that every stroke of paint adds vibrancy and life to your home. Whether it's a fresh coat to revitalize your interiors or a bold color choice to make a statement, Nimesha's meticulous workmanship and dedication to customer satisfaction guarantee a stunning finish that enhances the beauty of your home in Hapugala, Galle. Trust Nimesha De Silva for professional house painting services that exceed expectations.",
          "images": {
            "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwatm5nf02ym07pl5atbz3be"
          }
        },
        {
          "id": "clwdojxkn17vy07pi7mw2ku74",
          "name": "Amal Wijesinghe",
          "email": "malwijesinghe@example.com",
          "contactPerson": "Painting Services",
          "category": {
            "name": "Painting"
          },
          "address": "789, Beach Road, Galle",
          "about": "Amal Wijesinghe is the painting expert you can trust. With a keen eye for detail and a passion for perfection, Amal transforms spaces into works of art.",
          "images": {
            "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdojkw317vj07pi69f6qv2x"
          }
        },
        {
          "id": "clwdop4cy185507pidorqcxos",
          "name": "Tharindu Alwis",
          "email": "tharinduwickrama@example.com",
          "contactPerson": "Painting Services",
          "category": {
            "name": "Painting"
          },
          "address": "333, Riverside Road, Anuradhapura",
          "about": "Tharindu Wickramasinghe is passionate about painting. With creative flair and precision, Tharindu transforms walls into masterpieces.",
          "images": {
            "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdooq9g17lz07pom4t7hofo"
          }
        }
      ]
    }

    const Shifting = {
      "businessLists": [
        {
          "id": "clwatovd9031c07o6f37xq3wo",
          "name": "Nimal Alponsu",
          "email": "alponsu.nimal@gmail.com",
          "contactPerson": "Delivery Service",
          "category": {
            "name": "Shifting"
          },
          "address": "Kadawatha, Colombo",
          "about": "Mimal Alponsu offers reliable delivery services for any item in the bustling city of Kadawatha, Colombo. With a focus on efficiency and timeliness, Mimal ensures that your packages, parcels, and goods reach their destination safely and on schedule. Whether it's a small package or a bulky item, Mimal's commitment to customer satisfaction guarantees a seamless delivery experience. Trust Mimal Alponsu for swift and dependable delivery services that cater to your needs in Kadawatha, Colombo.",
          "images": {
            "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwatnocy02zq07o6ll3ktyr3"
          }
        },
        {
          "id": "clwdomq6x17il07porxidy920",
          "name": "Kusal Rajapaksa",
          "email": "kusalrajapaksa@example.com",
          "contactPerson": "Delivery Services",
          "category": {
            "name": "Shifting"
          },
          "address": "222, Hillside Avenue, Matale",
          "about": "Kusal Rajapaksa is your go-to delivery expert. With a commitment to efficiency and reliability, Kusal ensures your parcels are delivered with care.",
          "images": {
            "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdom8z217zl07pimvvipat3"
          }
        },
        {
          "id": "clwdou2kz17ra07po095lk8b0",
          "name": "Upul Gunasekara",
          "email": "pulgunasekara@example.com",
          "contactPerson": "Delivery Services",
          "category": {
            "name": "Shifting"
          },
          "address": "555, Mountain View Drive, Badulla",
          "about": "Upul Gunasekara is your trusted delivery partner. With speed and reliability, Upul ensures your packages are delivered with care and efficiency.",
          "images": {
            "url": "https://ap-south-1.graphassets.com/clw64u3om0c670dpmgj3zdqrk/clwdots4017qv07pocwnz3qa0"
          }
        }
      ]
    }

    if (category == "Cleaning") {return Cleaning;}
    else if (category == "Repairing") {return Repairing;}
    else if (category == "Painting") {return Painting;}
    else if (category == "Shifting") {return Shifting;}
}



export default { getSlider, getCategories, getBusinessList, getBusinessListByCategory };
